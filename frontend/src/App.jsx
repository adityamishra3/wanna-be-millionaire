...
function AuthScreen({ onAuth }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const user = await apiFetch(`/auth/${mode}`, {
        method: "POST",
      ...
      >
        <div style={{ marginBottom: 36, textAlign: "center" }}>
          ...
          >
            Wanna Be
            <br />
            <span style={{ color: "#e8d5a3" }}>Millionaire</span>
          </div>
        </div>
...
function IdeaForm({ onSave, onCancel, notify, initial }) {
  const [form, setForm] = useState(
    initial || { title: "", content: "", isPublic: false },
  );
  const [loading, setLoading] = useState(false);
  ...
  async function submit(e) {
    ...
    try {
      if (isEdit) {
        await apiFetch(`/idea/${initial.id}`, {
          method: "PATCH",
    ...
    >
      ...
      >
        <Input
          ...
          value={form.title}
          onChange={set("title")}
          placeholder="Your million dollar idea..."
        />
        <div>
          <label
            style={{
        ...
        >
          <div
            style={{
              ...
              boxShadow: idea.isPublic
                ? "0 0 8px rgba(110,231,183,0.4)"
                : "none",
            }}
          />
          {!readonly && hovered && (
...
function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    // websocket listeners
    socket.on("new_public_idea", (idea) => {
      setPublicIdeas((prev) => [idea, ...prev]);
    });

    socket.on("delete_public_idea", (ideaId) => {
      setPublicIdeas((prev) => {
  ...
  return (
    <div
      style={{ minHeight: "100vh", background: "#0a0a0a", color: "#f5f0e8" }}
    >
      <div
        style={{
          position: "fixed",
          ...
          {tabs.map((t) => (
            <button
              ...
              style={{
                ...
                transition: "all 0.2s",
                background:
                  tab === t.id ? "rgba(232,213,163,0.12)" : "transparent",
            ...
            {!showForm && !editingIdea && (
              <button
                ...
                onMouseEnter={(e) => {
                  ...
                  e.currentTarget.style.background = "rgba(232,213,163,0.03)";
                }}
                onMouseLeave={(e) => {
